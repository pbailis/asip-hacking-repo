...
ALGORITHMS = ["ADMM", "MiniBatchADMM" "AsyncADMM", "PORKCHOP"]#, "HOGWILD", "GD", "PORKCHOP"]
ALGORITHMS = ["ADMM", "AsyncADMM", "PORKCHOP"]#, "HOGWILD", "GD", "PORKCHOP"]
...
GLOBAL_ADMMlocalEpsilon = 1.0e-4
GLOBAL_ADMMrho = 2.0
GLOBAL_ADMMlagrangianRho = 2.0
...
def runTest(runtimeMS,
            ...
            ADMMlocalEpsilon = GLOBAL_ADMM_localEpsilon,
...
for dataset in ["wikipedia", "bismarck", "dblp"]: #, "flights"]:
    for runtime in RUNTIMES:
        for algorithm in ALGORITHMS:
            broadcastDelay = -1
            localEpsilon = GLOBAL_ADMM_localEpsilon
            miscStr = " --useLineSearch true --miniBatchSize 1000"
            if algorithm == "ADMM":
                maxLocalIterations = GLOBAL_ADMM_maxLocalIterations
                localEpsilon = GLOBAL_ADMM_localEpsilon
            elif algorithm == "MiniBatchADMM":
