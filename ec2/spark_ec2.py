...
def launch_cluster(conn, opts, cluster_name):
    ...
    if opts.security_group_prefix is None:
        ...
        main_group = get_or_make_group(conn, opts.security_group_prefix + "-main")
        subordinate_group = get_or_make_group(conn, opts.security_group_prefix + "-subordinates")
        ...
        try:
            while True:
                ...
                if len(active_instance_ids) == opts.subordinates:
                    ...
                    print "%d of %d subordinates granted, waiting longer for request ids including %s" % (
                        len(active_instance_ids), opts.subordinates, outstanding_request_ids[0:10])
        except:
    ...
    for main in main_nodes:
        name = '{cn}-main-{iid}'.format(cn=cluster_name, iid=main.id)
        tag_instance(main, name)

    for subordinate in subordinate_nodes:
        name = '{cn}-subordinate-{iid}'.format(cn=cluster_name, iid=subordinate.id)
        tag_instance(subordinate, name)
...
def get_existing_cluster(conn, opts, cluster_name, die_on_error=True):
    ...
    # Now proceed to detect main and subordinates instances.
    ...
    for res in reservations:
        ...
        for inst in active:
            ...
            if name.startswith(cluster_name + "-main"):
                main_nodes.append(inst)
            elif name.startswith(cluster_name + "-subordinate"):
    ...
    if main_nodes != [] or not die_on_error:
        ...
        if main_nodes == [] and subordinate_nodes != []:
            print >> sys.stderr, "ERROR: Could not find main in with name " + \
                cluster_name + "-main"
        else:
...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key):
    ...
    ssh(main, opts, "rm -rf spark-ec2 && git clone https://github.com/mesos/spark-ec2.git -b v4")

    print "Deploying files to main..."
    deploy_files(conn, "deploy.generic", opts, main_nodes, subordinate_nodes, modules)
        ...
        if response == "y":
            ...
            if opts.delete_groups:
                ...
                if opts.security_group_prefix is None:
                    ...
                    group_names = [opts.security_group_prefix + "-main",
                                   opts.security_group_prefix + "-subordinates"]
                wait_for_cluster_state(
                    cluster_instances=(main_nodes + subordinate_nodes),
